...
class _JustificationScreenState extends State<JustificationScreen> {
  ...
  Future<void> _loadTypes() async {
    try {
      final types = await Provider.of<AuthProvider>(context, listen: false).getJustificationTypes();
      setState(() {
        _types = types;
  ...
  Future<void> _submit() async {
    ...
    if (_attachedFile == null) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Debe adjuntar una evidencia.')));
       return;
    }

    try {
      await Provider.of<AuthProvider>(context, listen: false).createJustification(
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  decoration: const InputDecoration(labelText: 'Tipo de Justificación', border: OutlineInputBorder()),
                  items: _types.map((t) => DropdownMenuItem(value: t.id.toString(), child: Text(t.nombre))).toList(),
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Motivo / Detalle', border: OutlineInputBorder(), alignLabelWithHint: true),
                  maxLines: 4,
                  validator: (v) => v!.isEmpty ? 'Requerido' : null,
                ),
                const SizedBox(height: 20),
                const Text("Adjuntar Evidencia", style: TextStyle(fontWeight: FontWeight.bold)),
                ...
                if (_attachedFile != null) ...[
                   ...
                   Chip(
                     ...
                     avatar: Icon(_isPdf ? Icons.picture_as_pdf : Icons.image),
     ...
     return Row(
       children: [
         Expanded(
           child: InkWell(
             ...
             child: InputDecorator(
               decoration: const InputDecoration(labelText: 'Desde', border: OutlineInputBorder()),
         ...
         Expanded(
           child: InkWell(
             onTap: () async {
               final date = await showDatePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime(2030), initialDate: _endDate);
               if (date != null) setState(() => _endDate = date);
             },
             child: InputDecorator(
               decoration: const InputDecoration(labelText: 'Hasta', border: OutlineInputBorder()),
