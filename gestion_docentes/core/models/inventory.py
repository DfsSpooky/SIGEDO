...
class TipoActivo(models.Model):
    nombre = models.CharField(
        max_length=100, unique=True, help_text="Ej: Laptop, Proyector, Monitor"
    )
...
class Activo(models.Model):
    ...
    tipo = models.ForeignKey(
        TipoActivo, on_delete=models.PROTECT, related_name="activos"
    )
...
class Reserva(models.Model):
    ...
    activo = models.ForeignKey(
        Activo, on_delete=models.CASCADE, related_name="reservas"
    )
    # String references
    docente = models.ForeignKey(
        "core.Docente", on_delete=models.CASCADE, related_name="reservas"
    )
    ...
    franja_horaria_inicio = models.ForeignKey(
        "core.FranjaHoraria",
    ...
    franja_horaria_fin = models.ForeignKey(
        "core.FranjaHoraria",
        on_delete=models.PROTECT,
    ...
    fecha_confirmacion = models.DateTimeField(
        null=True, blank=True, help_text="Momento en que se recoge el equipo."
    )
    fecha_finalizacion = models.DateTimeField(
        null=True, blank=True, help_text="Momento en que se devuelve el equipo."
    )
    ...
    def save(self, *args, **kwargs):
        # Changed import from 'core.models' to '.communication' to avoid circular import if necessary,
        # or stick to absolute import if the package is fully loaded.
        # Since 'core.models' imports everything in __init__.py, importing from it inside a function IS safe.
        # However, importing from sibling is cleaner.
        from django.urls import reverse

        from .communication import Notificacion
