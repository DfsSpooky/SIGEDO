...
def _generar_datos_reporte_asistencia(filters):
    ...
    asistencias_en_rango = Asistencia.objects.filter(
        fecha__range=[fecha_inicio, fecha_fin]
    ).select_related("docente", "curso")
    asistencias_diarias_en_rango = AsistenciaDiaria.objects.filter(
        fecha__range=[fecha_inicio, fecha_fin]
    )

    # La fuente de verdad ahora son los Bloques de Horario
    bloques_programados_qs = BloqueHorario.objects.filter(
    ...
    if curso_id:
        ...
        if not docente_id:
            docentes_del_curso_ids = bloques_programados_qs.values_list(
                "curso__docente_id", flat=True
            ).distinct()
    ...
    asistencias_diarias_map = {
        (ad.docente_id, ad.fecha): ad for ad in asistencias_diarias_en_rango
    }
    ...
    justificaciones_aprobadas = Justificacion.objects.filter(
        estado="APROBADO", fecha_inicio__lte=fecha_fin, fecha_fin__gte=fecha_inicio
    )
        ...
        for dia_actual in dias_del_rango:
            ...
            if asistencias_del_dia:
                ...
                for asis in asistencias_del_dia:
                    ...
                    bloque_asistencia = next(
                        (b for b in bloques_del_dia if b.curso_id == asis.curso_id),
                        None,
                    )
                    ...
                    if asis.hora_entrada and bloque_asistencia:
                        hora_inicio_dt = timezone.make_aware(
                            timezone.datetime.combine(
                                dia_actual, bloque_asistencia.horario_inicio
                            )
                        )
            ...
            if (
                estado_dia == "Falta"
                and (docente.id, dia_actual) in justificaciones_set
            ):
                estado_dia = "Justificado"

            if estado_filtro == "todos" or estado_dia.lower() == estado_filtro:
                asistencia_diaria = asistencias_diarias_map.get(
                    (docente.id, dia_actual)
                )
                reporte_final.append(
                    {
                        "docente": docente,
