...
STYLES.add(
    ParagraphStyle(name="TableCellCenter", parent=STYLES["TableCell"], alignment=1)
)
STYLES.add(
    ParagraphStyle(
        name="TableCellSmall",
...
STYLES.add(
    ParagraphStyle(
        ...
        backColor=colors.mediumseagreen,
...
class ReportePDFTemplate(BaseDocTemplate):
    def __init__(self, filename, **kwargs):
        ...
        template = PageTemplate(
            id="main_template", frames=[frame], onPage=self._draw_header_footer
        )
    ...
    def _draw_header_footer(self, canvas, doc):
        ...
        if (
            self.configuracion
            and self.configuracion.logo
            and hasattr(self.configuracion.logo, "path")
        ):
            try:
                logo_img = Image(
                    self.configuracion.logo.path,
                    width=1.2 * inch,
                    height=1.2 * inch,
                    hAlign="CENTER",
                )
        ...
        nombre_institucion = Paragraph(
            (
                self.configuracion.nombre_institucion
                if self.configuracion
                else "Nombre de Institución"
            ),
            STYLES["InstitutionTitle"],
        )
        if self.configuracion and self.configuracion.facultad:
            nombre_facultad = Paragraph(
                self.configuracion.facultad.nombre.upper(), STYLES["FacultyTitle"]
            )
        else:
            nombre_facultad = Spacer(0, 0)
        titulo_reporte = Paragraph(
            "REPORTE DE ASISTENCIA DOCENTE", STYLES["ReportTitle"]
        )
        periodo_reporte = Paragraph(
            f"Periodo del {self.fecha_inicio} al {self.fecha_fin}",
        ...
        header_table = Table(
            [[logo_img, header_text_content]], colWidths=[1.5 * inch, 8.0 * inch]
        )
        ...
        header_table.drawOn(
            canvas, self.leftMargin, self.height + self.topMargin - h + 0.3 * inch
        )

        canvas.setStrokeColorRGB(0.8, 0.8, 0.8)
        canvas.line(
            self.leftMargin,
            self.height + self.topMargin - h + 0.2 * inch,
            self.width + self.leftMargin,
            self.height + self.topMargin - h + 0.2 * inch,
        )

        fecha_generacion = Paragraph(
            f"Generado el: {timezone.localtime(timezone.now()).strftime('%d/%m/%Y %I:%M:%S %p')}",
            STYLES["TableCellSmall"],
        )
        numero_pagina = Paragraph(
            f"Página {doc.page} de {self.page_count}", STYLES["TableCellSmall"]
        )
        footer_table = Table(
            [[fecha_generacion, numero_pagina]],
...
def exportar_reporte_pdf(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="Reporte_Asistencia_{fecha_inicio_str}_a_{fecha_fin_str}.pdf"'
    )
    buffer = BytesIO()
    ...
    for record in reporte_final:
        docente_cell = Paragraph(
            f"{record['docente'].last_name}, {record['docente'].first_name}",
            STYLES["TableCell"],
        )
        fecha_cell = Paragraph(
            record["fecha"].strftime("%d/%m/%Y"), STYLES["TableCellCenter"]
        )
        estado_cell = Paragraph(
            record["estado"],
            status_styles.get(record["estado"], STYLES["TableCellCenter"]),
        )

        detalles_cells = []
        if record["asistencia_diaria"]:
            hora_general_str = (
                record["asistencia_diaria"]
                .hora_entrada.astimezone(peru_tz)
                .strftime("%I:%M:%S %p")
            )
            detalles_cells.append(
                Paragraph(
                    f"<b>Asistencia General: {hora_general_str}</b>",
                    STYLES["TableCellSmall"],
                )
            )
        ...
        if record["asistencias"]:
            for asis in record["asistencias"]:
                hora_entrada_str = (
                    asis.hora_entrada.astimezone(peru_tz).strftime("%I:%M:%S %p")
                ...
                bloque_asistencia = BloqueHorario.objects.filter(
                    curso=asis.curso, dia_semana=asis.fecha.weekday()
                ).first()
                if asis.hora_entrada and bloque_asistencia:
                    limite_tardanza = timedelta(
                        minutes=(
                            ConfiguracionInstitucion.load().tiempo_limite_tardanza or 10
                        )
                    )
                    hora_inicio_dt = timezone.make_aware(
                        timezone.datetime.combine(
                            asis.fecha, bloque_asistencia.horario_inicio
                        )
                    )
    ...
    table = Table(
        table_data,
        colWidths=[1.5 * inch, 3 * inch, 1.5 * inch, 3.5 * inch],
        repeatRows=1,
    )
    table.setStyle(
        TableStyle(
...
def exportar_reporte_excel(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="Reporte_Asistencia_{fecha_inicio_str}_a_{fecha_fin_str}.xlsx"'
    )
    workbook = Workbook()
    ...
    for record in reporte_final:
        ...
        if record["asistencias"]:
            for asis in record["asistencias"]:
                hora_entrada_str = (
                    asis.hora_entrada.astimezone(peru_tz).strftime("%I:%M %p")
                    if asis.hora_entrada
                    else "--:--"
                )
                ...
                if asis.hora_entrada and bloque_asistencia:
                    hora_inicio_dt = timezone.make_aware(
                        timezone.datetime.combine(
                            record["fecha"], bloque_asistencia.horario_inicio
                        )
                    )
                    if (asis.hora_entrada - hora_inicio_dt) > timedelta(
                        minutes=limite_tardanza
                    ):
                        detalle += " (TARDE)"
        ...
        worksheet.append(
            [fecha_str, docente_nombre, docente_dni, estado_str, detalles_str]
        )
...
def exportar_ficha_docente_pdf(docente, semestre_activo):
    ...
    styles.add(
        ParagraphStyle(name="FichaBody", fontName="Helvetica", fontSize=10, leading=14)
    )
    styles.add(
        ParagraphStyle(
            name="FichaBodyBold", parent=styles["FichaBody"], fontName="Helvetica-Bold"
        )
    )
    ...
    configuracion = ConfiguracionInstitucion.load()
    logo_img = Spacer(0, 0)
    if configuracion.logo and configuracion.logo.storage.exists(
    ...
    header_data = [
        [
            logo_img,
            Paragraph(
                f"<b>{configuracion.nombre_institucion}</b><br/>{configuracion.facultad.nombre if configuracion.facultad else ''}",
                styles["FichaBody"],
            ),
        ]
    ]
    ...
    personal_data = [
        [
            Paragraph("<b>Nombre Completo:</b>", styles["FichaBody"]),
            Paragraph(docente.get_full_name(), styles["FichaBody"]),
        ],
        [
            Paragraph("<b>DNI:</b>", styles["FichaBody"]),
            Paragraph(docente.dni, styles["FichaBody"]),
        ],
        [
            Paragraph("<b>Email:</b>", styles["FichaBody"]),
            Paragraph(docente.email, styles["FichaBody"]),
        ],
    ]
    ...
    elements.append(
        Paragraph(
            f"2. Carga Académica ({semestre_activo.nombre})", styles["FichaSubtitle"]
        )
    )
    ...
    if cursos.exists():
        ...
        elements.append(
            Paragraph(
                "No tiene cursos asignados en este semestre.", styles["FichaBody"]
            )
        )
    elements.append(Spacer(1, 0.25 * inch))
    ...
    total_clases_programadas = Asistencia.objects.filter(
        docente=docente, curso__in=cursos
    ).count()
    asistencias_contadas = Asistencia.objects.filter(
        docente=docente, curso__in=cursos, hora_entrada__isnull=False
    ).count()
    porcentaje_asistencia = (
        (asistencias_contadas / total_clases_programadas * 100)
    ...
    desempeno_data = [
        [
            Paragraph("<b>Clases Programadas en el Semestre:</b>", styles["FichaBody"]),
            Paragraph(str(total_clases_programadas), styles["FichaBody"]),
        ],
        [
            Paragraph("<b>Asistencias Registradas:</b>", styles["FichaBody"]),
            Paragraph(str(asistencias_contadas), styles["FichaBody"]),
        ],
        [
            Paragraph("<b>Porcentaje de Asistencia:</b>", styles["FichaBody"]),
            Paragraph(f"{porcentaje_asistencia:.2f}%", styles["FichaBodyBold"]),
        ],
    ]
    ...
    doc_data = [
        [
            Paragraph(f"<b>{d.get_estado_display()}</b>", styles["FichaBody"]),
            Paragraph(d.titulo, styles["FichaBody"]),
        ]
        for d in documentos
    ]
    if doc_data:
        ...
        elements.append(
            Paragraph("No hay documentos registrados.", styles["FichaBody"])
        )
