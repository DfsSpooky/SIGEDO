    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(25):
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Docente creado: {docente.username} (DNI: {docente.dni})"
                    )
                )

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"Error al crear docente {username}: {e}")
                )

        self.stdout.write(
            self.style.SUCCESS(
                "¡Proceso finalizado! Se crearon los docentes de prueba."
            )
        )
