...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        reservas_pendientes = Reserva.objects.filter(
            estado="RESERVADO", fecha_reserva__lte=now.date()
        ).select_related("activo", "franja_horaria")

        reservas_expiradas_count = 0

        self.stdout.write(
            f"Verificando reservas a las {now.strftime('%Y-%m-%d %I:%M:%S %p')}..."
        )
        ...
        for reserva in reservas_pendientes:
            ...
            hora_inicio_reserva = timezone.make_aware(
                timezone.datetime.combine(
                    reserva.fecha_reserva, reserva.franja_horaria.hora_inicio
                ),
            ...
            if now > momento_expiracion:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f'Reserva ID {reserva.id} para "{reserva.activo.nombre}" ha expirado y fue cancelada.'
                    )
                )
        ...
        if reservas_expiradas_count > 0:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Proceso finalizado. Se cancelaron {reservas_expiradas_count} reservas expiradas."
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    "No se encontraron reservas expiradas para cancelar."
                )
            )
